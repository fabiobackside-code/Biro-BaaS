...
{
    x.AddConsumer<DebitCommandConsumer>();
...
app.MapPost("/debit", async ([FromBody] DebitRequest request, IBus bus) =>
{
    var transactionId = NewId.NextGuid();
    await bus.Publish(new DebitCommand { TransactionId = transactionId, AccountId = request.AccountId, Amount = request.Amount, WebhookUrl = request.WebhookUrl });
